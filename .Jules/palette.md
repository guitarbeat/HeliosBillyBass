...
## 2025-11-21 - [Accessible Tooltips as Disclosures]
**Learning:** Tooltips that toggle visibility on click are functionally disclosure widgets, not just tooltips. Simple `display: block` toggling is insufficient; the trigger button must use `aria-expanded` and `aria-controls` to communicate state and relationship to screen readers.
**Action:** Implemented unique ID generation for tooltips and updated the trigger button to include `aria-expanded` and `aria-controls`. Updated the toggle script to manage the ARIA state dynamically.
